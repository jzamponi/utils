...
def plot_opacity(filename, show=True, savefig=None, return_data=False):
    """ Plot the dust opacity file generated by RADMC3D """
    ...
    if return_data:
        return {'lam': l, 'ext': k_ext, 'sca': k_sca, 'abs': k_abs}

    return plot_checkout(fig, show, savefig)


def plot_polaris_opacity(
...
def dropdeg(fig, dropaxis=2):
    ...
    """
    temp_wcs = fig._wcs.dropaxis(dropaxis)
...
def radmc3d_writer(snapshot, source='athena'):
    """ 
        Read in simulation snapshots with python-yt and 
        create a radmc3d dust density file
    """

    import yt
    from yt.extensions.astro_analysis.radmc3d_export.api \
        import RadMC3DWriter, RadMC3DSource
    ...
    print_(f'Reading simulation snapshot: {snapshot}')
    ds = yt.load(snapshot)
    writer = RadMC3DWriter(ds)

    print_(f'Writing amr_grid.inp')
    ...
    ds.add_field(
        ("gas", "dust_density"), 
        sampling_type="local",
        function = lambda field, data: d2g * data['density'], 
        units = "g/cm**3"
    )
    print_(f'Writing dust_density.inp')
